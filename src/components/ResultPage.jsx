...
function buildWALink(number, childInfo, profile, topStrengths, lang) {
  const isAr = lang === 'ar';
  const isFr = lang === 'fr';

  const strengthNames = topStrengths
    .map(k => isAr
      ? profiles[k].strengthLabel_ar
      : isFr
        ? (profiles[k].strengthLabel_fr ?? profiles[k].strengthLabel)
        : profiles[k].strengthLabel
    )
    .join(', ');

  const planet = isAr ? profile.planet_ar : isFr ? (profile.planet_fr ?? profile.planet) : profile.planet;
  const path   = isAr ? profile.recommendedPath_ar : isFr ? (profile.recommendedPath_fr ?? profile.recommendedPath) : profile.recommendedPath;
...
أودّ حجز جلسة تجريبية مجانية في The Tech Camp. أرجو إرسال الأوقات المتاحة! 🚀`;
  ...
  const msgFr =
`Bonjour Coducators ! 👋 Je viens de compléter l'évaluation des compétences futures pour mon enfant.

👦 Prénom de l'enfant : ${childInfo.childName}
🎂 Âge : ${childInfo.childAge}
👤 Prénom du parent : ${childInfo.parentName}
🪐 Résultat de mission : ${planet}
⭐ Parcours recommandé : ${path}
💪 3 points forts principaux : ${strengthNames}

J'aimerais réserver une séance d'essai gratuite au Tech Camp. Envoyez-moi les créneaux disponibles ! 🚀`;

  const msg = isAr ? msgAr : isFr ? msgFr : msgEn;
...
export default function ResultPage({ childInfo, answers, onRestart }) {
  const { t, isAr, isFr, lang } = useLanguage();
  ...
  const waLink       = buildWALink(WHATSAPP_NUMBER, childInfo, profile, topStrengths, lang);

  // Pick the right language text from a profile field triple
  const pText = (en, fr, ar) => isAr ? ar : isFr ? (fr ?? en) : en;
    ...
    <div className="result-page" style={{ position: 'relative', zIndex: 10 }}>
      ...
      >
        ...
        <p style={{ fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: profile.color, marginBottom: 16 }}>
          {t('Mission Report — Complete', 'تقرير المهمة — مكتمل', 'Rapport de Mission — Terminé')}
        </p>
        ...
        <h1 className="result-planet-name" style={{ marginTop: 20 }}>
          {pText(profile.planet, profile.planet_fr, profile.planet_ar)}
        </h1>
        ...
        {childInfo.childName && (
          <div className="result-personal-headline">
            ...
            <span className="result-personal-sep">
              {t('is headed to', 'متجه نحو', 'est en route vers')}
            </span>
            <span className="result-personal-planet" style={{ color: profile.color }}>
              {pText(profile.planet, profile.planet_fr, profile.planet_ar)} {profile.icon}
            </span>
          </div>
        )}

        <p className="result-tagline">{pText(profile.tagline, profile.tagline_fr, profile.tagline_ar)}</p>
      </div>
      ...
      <div className="result-body">
        ...
        <div className="card result-section">
          <p className="result-section-title" style={{ color: profile.color }}>
            {t('Mission Analysis', 'تحليل المهمة', 'Analyse de Mission')}
          </p>
          <p>{pText(profile.shortResult, profile.shortResult_fr, profile.shortResult_ar)}</p>
        </div>

        {/* Top 3 strengths */}
        <div className="card result-section mt-12">
          <p className="result-section-title" style={{ color: profile.color }}>
            {t('Top Strengths Identified', 'أبرز المهارات المكتشفة', 'Points Forts Identifiés')}
          </p>
              ...
              <div key={key} className="strength-item">
                ...
                <div>
                  <p className="strength-name">
                    {pText(profiles[key].strengthLabel, profiles[key].strengthLabel_fr, profiles[key].strengthLabel_ar)}
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className="card result-section mt-12">
          <p className="result-section-title" style={{ color: profile.color }}>
            {t('Recommended Tech Camp Path', 'المسار الموصى به', 'Parcours Recommandé au Tech Camp')}
          </p>
          <div style={{
            ...
            🗺️ {pText(profile.recommendedPath, profile.recommendedPath_fr, profile.recommendedPath_ar)}
          </div>
          <div className="chips">
            {pText(profile.activities, profile.activities_fr, profile.activities_ar).map(a => (
              <span key={a} className="chip">✦ {a}</span>
            ))}
          </div>
          <p style={{ marginTop: 12, fontSize: '0.82rem', color: 'var(--text-muted)', fontStyle: 'italic' }}>
            {pText(profile.campMessage, profile.campMessage_fr, profile.campMessage_ar)}
          </p>
        </div>
        ...
        <div className="nudge-card" style={{ background: `rgba(${profile.color.replace('#','').match(/.{2}/g).map(h=>parseInt(h,16)).join(',')},0.08)`, borderColor: `${profile.color}30` }}>
          <p>
            {t(
              ...
              "Ce résultat donne un aperçu solide des points forts de ton enfant — mais la meilleure façon de découvrir son plein potentiel, c'est de le voir en action. La séance d'essai gratuite est conçue exactement pour ça."
            )}
          </p>
        </div>
        ...
        <div className="mt-24">
          <a href={waLink} target="_blank" rel="noopener noreferrer" className="btn btn-whatsapp">
            ...
            {t('📲 Book Free Trial on WhatsApp', '📲 احجز جلسة تجريبية مجانية على واتساب', '📲 Réserver un essai gratuit sur WhatsApp')}
          </a>
        </div>

        <div className="mt-12">
          <button className="btn btn-outline" onClick={onRestart}>
            {t('Retake the assessment', 'أعد التقييم', "Refaire l'évaluation")}
          </button>
        </div>
        ...
        <p className="disclaimer mt-16">
          {t(
            ...
            "Cette évaluation est conçue pour explorer les préférences d'apprentissage tech de ton enfant. Ce n'est pas une évaluation scientifique ou clinique."
          )}
        </p>
      </div>
